// Renderer-world bridge for the settings window.
//
// Holds the DOM, window-global, and IPC-event logic that used to live in
// settings-preload.js, but on the page's own `window` so React components,
// window.* reads/writes, and CustomEvents work under contextIsolation.
// `window.ipc` and the decoded launch args (window.ttBridge.jsVars) come from
