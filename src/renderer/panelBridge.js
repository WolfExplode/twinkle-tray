// Renderer-world bridge for the brightness panel window.
//
// Holds the DOM, window-global, and IPC-event logic that used to live in
// panel-preload.js, but on the page's own `window` so React components,
// window.* reads/writes, and CustomEvents work under contextIsolation.
// Node-privileged calls (priority, gc, launch args) go through window.ttBridge,
// exposed by the preload via contextBridge.
import StackBlur from 'stackblur-canvas'

const ipc = window.ipc
const bridge = window.ttBridge

// Send page logs to the main process logger.
const con = {
    log: console.log,
    warn: console.warn,
    error: console.error
