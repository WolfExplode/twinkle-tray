...
// One interpolation step of a focus transition. Brightness is rounded (it drives
// a DDC write); software dim stays fractional for a smooth overlay fade.
function computeTransitionStep({ startBrightness, targetBrightness, startSoftwareDim, targetSoftwareDim, progress }) {
  const p = Math.min(1, Math.max(0, progress))
  return {
    brightness: Math.round(startBrightness + (targetBrightness - startBrightness) * p),
    softwareDim: startSoftwareDim + (targetSoftwareDim - startSoftwareDim) * p
  }
}
